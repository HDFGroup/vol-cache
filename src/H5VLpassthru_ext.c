...
H5Dread_to_cache(hid_t dset_id, hid_t mem_type_id, hid_t mem_space_id,
    ...
    void **req; 
    assert(-1 != H5VL_passthru_dataset_read_to_cache_op_g);
	printf("mem_type_id outside: %ld\n", mem_type_id);
	printf("mem_space_id outside: %ld\n", mem_space_id); 
  	printf("file_space_id outside: %ld\n", file_space_id); 
  	printf("plist_id outside: %ld\n", plist_id);
...
H5VL_pass_through_ext_init(hid_t vipl_id)
{
  ...
  printf("------- EXT PASS THROUGH VOL INIT\n");
    ...
    // get the path 
    if (getenv("LOCAL_STORAGE_PATH")) 
        strcpy(ls_path, getenv("LOCAL_STORAGE_PATH")); 
    else 
        strcpy(ls_path, "./"); 
    
    // get the storage size
    if (getenv("LOCAL_STORAGE_SIZE"))
        ls_size = atof(getenv("LOCAL_STORAGE_SIZE"))*1073741824; 
    
    // get the storate type
    if (getenv("LOCAL_STORAGE_TYPE")) {
      if (strcmp(getenv("LOCAL_STORAGE_TYPE"), "SSD")==0) {
            ls_type = SSD;
      } else if (strcmp(getenv("LOCAL_STORAGE_TYPE"), "BURST_BUFFER")==0) {
            ls_type = BURST_BUFFER; 
      }  else if (strcmp(getenv("LOCAL_STORAGE_TYPE"), "MEMORY")==0) {
            ls_type = MEMORY;
      }
      printf("LOCAL_STORAGE_TYPE: %s", getenv("LOCAL_STORAGE_TYPE"));
    }
    WRITE_CACHE_SIZE = 1073741824; //setting the default cache size to be 1 GB; 
    if (getenv("WRITE_CACHE_SIZE")) {
      WRITE_CACHE_SIZE = atof(getenv("WRITE_CACHE_SIZE"));
    }
...
H5Dcreate_mmap_win(void *obj, const char *prefix) {
  ...
  if (H5LS.storage!=MEMORY) {
    ...
    strcpy(dset->H5DRMM->mmap.fname, prefix); 
...
static herr_t 
H5VL_pass_through_ext_dataset_read_cache_setup(void *obj, void *loc, const char *name)
{
	printf("read_cache_setup\n");
    H5VL_pass_through_ext_t *dset = (H5VL_pass_through_ext_t *) obj;
    H5VL_pass_through_ext_t *o = (H5VL_pass_through_ext_t *) loc;
    if (dset->H5DRMM==NULL) dset->H5DRMM = (H5Dread_cache_metadata *) malloc(sizeof(H5Dread_cache_metadata));
    hid_t unused; 
    hid_t dapl_id;
    void **req; 
  ...
  printf("Claim space\n");
    if (H5LSclaim_space(&H5LS, dset->H5DRMM->dset.size, HARD) == SUCCEED) { 
      ...
      mkdir(dset->H5DRMM->cache->path, 0755); // setup the folder with the name of the file, and put everything under it.
	    printf("register_cache\n");


      H5LSregister_cache(&H5LS, dset->H5DRMM->cache);
	  printf("register_cache done\n");
	  printf("mmap enter\n");
      H5Dcreate_mmap_win((void*)dset, name);
	  printf("mmap_win done\n");
      int rc = pthread_create(&dset->H5DRMM->io.pthread, NULL, H5Dread_pthread_func_vol, dset->H5DRMM);
      free(gdims);
      dset->read_cache_info_set = true;
	  printf("read_cache_setup done\n");
      return SUCCEED;
    } else {
      if (dset->H5DRMM->mpi.rank==0) printf("Unable to allocate space to the dataset for cache; read cache function will be turned off\n");
        dset->read_cache; 
        return FAIL; 
    }
}
    ...
    const char *name, hid_t dapl_id, hid_t dxpl_id, void **req)
{
    ...
    if(under) {
	  ...
	  /* setup read cache */
		  ...
		  H5VL_pass_through_ext_dataset_read_cache_setup(dset, obj, name);
		}
        /* Check for async request */
        if(req && *req)
			*req = H5VL_pass_through_ext_new_obj(*req, o->under_vol_id);
...
void *H5Dread_pthread_func_vol(void *args) {
  ...
  while (!dmm->io.dset_cached) {
    if (!dmm->io.batch_cached) {
	  ...
	  MPI_Allreduce(&dset_cached, &dmm->io.dset_cached, 1, MPI_C_BOOL, MPI_LAND, dmm->mpi.comm_t);
    ...
    hid_t file_space_id, hid_t plist_id, void *buf, void **req)
{
    ...
    bool contig = false;
    BATCH b;
    if (debug_level()>1) printf("Rank %d: dataset_read_from_cache\n", o->H5DRMM->mpi.rank);
...
printf("foo: i = %d, d = %f\n", i, d);
        ...
        pthread_mutex_unlock(&o->H5DRMM->io.request_lock);
        ret_value = H5VL_pass_through_ext_dataset_read_to_cache(obj, mem_type_id, mem_space_id, file_space_id, plist_id, buf, req);
        ...
        void *buf = va_arg(arguments, void *);
        void **req;
        ret_value = H5VL_pass_through_ext_dataset_read_from_cache(obj, mem_type_id, mem_space_id, file_space_id, plist_id, buf, req);
...
H5VL_pass_through_ext_dataset_close(void *dset, hid_t dxpl_id, void **req)
{
	...
	H5TSmutex_acquire(&acq);
      ...
      if (H5LS.storage!=MEMORY) {
        ...
        //remove(o->H5DRMM->mmap.fname); now included in H5LSremove_cache
      } else {
        free(o->H5DRMM->mmap.buf);
      }
      H5LSremove_cache(&H5LS, o->H5DRMM->cache);
    }
...
//H5VL_pass_through_ext_file_create_cache()

/*-------------------------------------------------------------------------
    ...
    hid_t fapl_id, hid_t dxpl_id, void **req)
{
    ...
    if (file->write_cache)
        ...
        if (H5LSclaim_space(&H5LS, WRITE_CACHE_SIZE, HARD) == FAIL) {
            printf("Unable to claim cache space for this file; Not using cache \n"); 
            file->write_cache = false; 
        }
    if (file->write_cache) {
        srand(time(NULL));   // Initialization, should only be called once.
        file->H5DWMM = (H5Dwrite_cache_metadata*) malloc(sizeof(H5Dwrite_cache_metadata)); // this is to 
        ...
        file->H5DWMM->cache->mspace_total = WRITE_CACHE_SIZE; 
        ...
        strcpy(file->H5DWMM->cache->path, H5LS.path);
        strcat(file->H5DWMM->cache->path, "/");
        strcat(file->H5DWMM->cache->path, name);
        strcat(file->H5DWMM->cache->path, "/");
        mkdir(file->H5DWMM->cache->path, 755); // setup the folder with the name of the file, and put everything under it.
        if (H5LSregister_cache(&H5LS, file->H5DWMM->cache)==FAIL) {printf("Could not register cache for file %s\n", name);};
    ...
    hid_t dxpl_id, void **req)
{
	...
	if (getenv("READ_CACHE"))
	  if (strcmp(getenv("READ_CACHE"), "no")==0)
	    file->read_cache=false;
	  if (file->read_cache) {
		if (file->H5DRMM==NULL) file->H5DRMM = (H5Dread_cache_metadata *) malloc(sizeof(H5Dread_cache_metadata));
		MPI_Comm comm;
		MPI_Info info_mpi; 
		...
		strcpy(file->H5DRMM->cache->path, H5LS.path);
		strcat(file->H5DRMM->cache->path, "/");
		strcat(file->H5DRMM->cache->path, name);
    ...
    hid_t dxpl_id, void **req, va_list arguments)
{
    ...
    if (opt_type == H5VL_passthru_file_reserve_cache_op_g) {
      ...
      if (o->write_cache) {
		...
		if (H5LSclaim_space(&H5LS, size, mode)>-1) {
		  if (o->H5DWMM==NULL) o->H5DWMM = (H5Dwrite_cache_metadata*) malloc(sizeof(H5Dwrite_cache_metadata));
		  o->H5DWMM->cache->mspace_total = size;
		  o->H5DWMM->cache->mspace_left = size;
		}
      }
      if (o->read_cache) {
		...
		if (H5LSclaim_space(&H5LS, size, mode)>-1) {
		  ...
		  o->H5DRMM->cache->mspace_total = size;
		  o->H5DWMM->cache->mspace_left = size;
		}
      }
      ret_value = 0;
      ...
      if (NULL != o->H5DWMM) {
		...
		*size = o->H5DWMM->cache->mspace_total;
		...
		printf("o->H5DRMM\n");
		*size = o->H5DRMM->cache->mspace_total;
      }
      else *size = 0; 
...
H5VL_pass_through_ext_file_close(void *file, hid_t dxpl_id, void **req)
{
    ...
    if (o->write_cache) {
      ...
      while(!acq)
	H5TSmutex_acquire(&acq);
      if (H5LSremove_cache(&H5LS, o->H5DWMM->cache)!=SUCCEED) {printf(" Could not remove cache %s\n", o->H5DWMM->cache->path);}; 
    }
    if (o->read_cache) {
      free(o->H5DRMM);
    }
    ...
    H5I_type_t *opened_type, hid_t dxpl_id, void **req)
{
	...
	if (*opened_type==H5I_GROUP) { // if group is opened 
	  ...
	  if (called) H5VL_pass_through_ext_dataset_read_cache_setup((void*) new_obj, o, loc_params->loc_data.loc_by_name.name);
	}
        /* Check for async request */
        if(req && *req)
            *req = H5VL_pass_through_ext_new_obj(*req, o->under_vol_id);
