 ...
 *-------------------------------------------------------------------------
 */
static herr_t H5Ssel_gather_copy(hid_t space, hid_t tid, const void *buf,
                                 void *mbuf, hsize_t offset) {
  // assert(false);
  ...
  for (i = 0; i < nseq; i++) {
    ...
    // CUDA_RUNTIME_API_CALL( cudaMemcpyAsync(&mp[offset+off_contig],
    // &p[off[i]], len[i], cudaMemcpyDeviceToHost, stream0 ) );
    // CUDA_RUNTIME_API_CALL( cudaMemPrefetchAsync(&mp[offset+off_contig],
    // len[i], 1, 0) );
    off_contig += len[i];
  }
  return 0;
...
static void *H5LS_GPU_write_buffer_to_mmap(hid_t mem_space_id,
...
static herr_t H5LS_GPU_create_write_mmap(MMAP *mm, hsize_t size) {
  ...
  // CUDA_RUNTIME_API_CALL(cudaDeviceGetAttribute (&result,
  ...
  // CUDA_RUNTIME_API_CALL( cudaMallocManaged(&mm->buf, size,
  // cudaMemAttachGlobal) );

  // CUDA_RUNTIME_API_CALL( cudaMemAdvise(mm->buf, size,
  // cudaMemAdviseSetPreferredLocation, device_id) ); CUDA_RUNTIME_API_CALL(
  // cudaMemAdvise(mm->buf, size, cudaMemAdviseSetAccessedBy, device_id) );
  // CUDA_RUNTIME_API_CALL( cudaMemAdvise(mm->buf, size,
  // cudaMemAdviseSetAccessedBy, gpu_id) ); CUDA_RUNTIME_API_CALL(
  // cudaMemAdvise(&mm->buf, size, cudaMemAdviseSetPreferredLocation, gpu_id) );
...
static herr_t removeFolderFake(const char *path) { return 0; };
