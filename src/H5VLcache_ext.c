...
/* Typedefs */
/************/
void *H5Dread_cache_func_vol(void *args); 
/********************* */
/* Function prototypes */
/********************* */
/* Helper routines */
...
H5VL_cache_ext_init(hid_t vipl_id)
{
  ...
  printf("------- EXT CACHE VOL INIT\n");
    ...
    // initialize local storage struct
...
cache_storage_t get_storage_type_from_str(char *str) {
  if (!strcmp(str, "SSD"))
    ...
    return MEMORY; 
  else if (!strcmp(str, "BURST_BUFFER"))
    return BURST_BUFFER;
  else
...
herr_t readLSConf(char *fname, LocalStorage *LS) {
  ...
  while(fgets(line, 256, file) != NULL)
  {
	...
	LS->storage = get_storage_type_from_str(mac);
    }
  ...
  struct stat sb;
  if (LS->storage == MEMORY || ( stat(LS->path, &sb) == 0 && S_ISDIR(sb.st_mode))) {
    return 0; 
  } else {
...
H5VL_cache_ext_str_to_info(const char *str, void **_info)
{
    ...
    while (p->next != NULL)
      p=p->next; 
    p->H5LS = (LocalStorage *) malloc(sizeof(LocalStorage));
    ...
    // add another item to the H5LS_stack; 
...

/*-------------------------------------------------------------------------
 * Function:    H5VL_cache_ext_dataset_read_cache_create
 *
 ...
 */
static herr_t 
H5VL_cache_ext_dataset_read_cache_create(void *obj, const char *name)
{
  ...
  void **req;
  char fname[255];
  hid_t plist = H5Pcopy(H5P_DATASET_XFER_DEFAULT);
  file_get_wrapper(o->under_object, o->under_vol_id, H5VL_FILE_GET_NAME, plist, NULL, (int)H5I_FILE, size_f, fname, &ret_value);
  if (o->H5DRMM==NULL || o->H5DRMM->cache==NULL) {
    o->read_cache = true;
    hid_t fapl_id;
    file_get_wrapper(o->under_object, o->under_vol_id, H5VL_FILE_GET_FAPL, plist, NULL, &fapl_id);
  ...
  dataset_get_wrapper(dset->under_object, dset->under_vol_id, H5VL_DATASET_GET_TYPE, plist, NULL, &dset->H5DRMM->dset.h5_datatype);
  dset->H5DRMM->dset.esize = H5Tget_size(dset->H5DRMM->dset.h5_datatype);
  hid_t fspace;
  dataset_get_wrapper(dset->under_object, dset->under_vol_id, H5VL_DATASET_GET_SPACE, plist, NULL, &fspace);
  ...
  if (H5LSclaim_space(dset->H5DRMM->H5LS, dset->H5DRMM->dset.size, HARD, dset->H5DRMM->H5LS->replacement_policy) == SUCCEED) { 
    ...
    if (dset->H5DRMM->mpi.rank==io_node() && debug_level()>1) printf("Read cache created in %s\n", dset->H5DRMM->cache->path); 
    ...
    hid_t dcpl_id, hid_t dapl_id, hid_t dxpl_id, void **req)
{
    ...
    if (o->read_cache) {
      ...
      H5VL_cache_ext_dataset_read_cache_create(dset, name);
    }
...

/*-------------------------------------------------------------------------
 * Function:    H5Dread_cache_func_vol
 *
    ...
    hid_t file_space_id, hid_t plist_id, const void *buf, void **req)
{
	...
	o->H5DWMM->cache->mspace_per_rank_total = available;
      ...
      // write the buffer to the node-local storage or RAM
...

/*-------------------------------------------------------------------------
 * Function:    H5VL_cache_ext_dataset_cache_remove
 *
 ...
 */
static herr_t
H5VL_cache_ext_dataset_cache_remove(void *dset, hid_t dxpl_id, void **req)
{
#ifdef ENABLE_EXT_CACHE_LOGGING
    ...
    hid_t dxpl_id, void **req, va_list arguments)
{
	...
	ret_value = H5VL_cache_ext_dataset_mmap_remap(obj);
      ...
      ret_value = H5VL_cache_ext_dataset_read_cache_create(obj, name); 
...
H5VL_cache_ext_dataset_wait(void *dset) {
  H5VL_cache_ext_t *o = (H5VL_cache_ext_t *)dset;
  if (o->write_cache) {
    if (io_node()==o->H5DWMM->mpi.rank && debug_level()>0) printf("dataset_wait\n");
    double available = o->H5DWMM->cache->mspace_per_rank_total;
    H5VL_request_status_t status; 
    while((o->H5DWMM->io.current_request != NULL) && (o->num_request_dataset > 0)) {
      H5VL_cache_ext_request_wait(o->H5DWMM->io.current_request->req, INF, &status);
      o->H5DWMM->io.num_request--;
      H5VL_cache_ext_t *d = (H5VL_cache_ext_t *) o->H5DWMM->io.current_request->dataset_obj;
      d->num_request_dataset--; 
...

/*-------------------------------------------------------------------------
 * Function:    H5VL_cache_ext_file_cache_create
 *
...
H5VL_cache_ext_file_cache_create(void *obj, const char *name, hid_t fapl_id, 
  ...
  if (purpose == WRITE) {
    ...
    file->H5DWMM->cache->mspace_total = p->H5LS->write_cache_size;
    ...
    if (file->H5DRMM->mpi.rank == io_node()) printf("file cache created: %s\n", file->H5DRMM->cache->path);
    ...
    H5I_type_t *opened_type, hid_t dxpl_id, void **req)
{
	...
	if (*opened_type==H5I_GROUP) { // if group is opened 
	  ...
	  if (called && new_obj->read_cache)
	    H5VL_cache_ext_dataset_read_cache_create((void*) new_obj, loc_params->loc_data.loc_by_name.name);
	}
        /* Check for async request */
        if(req && *req)
            *req = H5VL_cache_ext_new_obj(*req, o->under_vol_id);
