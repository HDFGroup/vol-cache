/* 
   This is one of the unit test in Cache VOL. The goal of this test is to find out whether 
   the system support mmap or not. 
 */
#include "fcntl.h"
#include "mpi.h"
#include "string.h"
...
int remote_put(void *buf, int count, int src, int dist, void *mmap_p, MPI_Aint disp, MPI_Comm comm) {
  int rank; 
...
int remote_get(void *buf, int count, int rank, int src, int dist, void *mmap_p, MPI_Aint disp, MPI_Comm comm) {
  int tag = 0;
  char *p = (char *) mmap_p; 
  if (src==dist) {
    if (rank==src) 
...
void create_read_mmap(char *name, long unsigned int size,
  ...
  if (getenv("MPI_WIN")) {
    ...
    for(long unsigned int i=0; i<size/kb; i++) {
      ...
      disp = disp*kb; 
      if (verbose==1 and i%1024==1023) printProgress(float(i+1)*kb/size);
      remote_put(&p[0], kb, rank, (rank+1)%nproc, buf, disp, MPI_COMM_WORLD);
    }
  MPI_Info info;
  MPI_Win_create(buf, size, 1, MPI_INFO_NULL, MPI_COMM_WORLD, &win);
  int rank, nproc;
