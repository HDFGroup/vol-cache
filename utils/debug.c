...
void LOG_INFO(const char *app_file, const char *app_func, unsigned app_line, int rank, const char *str) {
#ifndef NDEBUG
  if (HDF5_CACHE_LOG_LEVEL >= INFO)
    if (rank >= 0)
      printf(" [CACHE VOL][INFO][%d] %s: %s \t <%s:%s:L%d>\n", rank, GET_TIME(), str, app_file, app_func, app_line);
    else if (HDF5_CACHE_RANK_ID == HDF5_CACHE_IO_NODE)
      printf(" [CACHE VOL][INFO] %s: %s \t <%s:%s:L%d>\n", GET_TIME(), str, app_file, app_func, app_line);
#endif
}

void LOG_ERROR(const char *app_file, const char *app_func, unsigned app_line, int rank, const char *str) {
#ifndef NDEBUG
...
void LOG_DEBUG(const char *app_file, const char *app_func, unsigned app_line, int rank, const char *str) {
#ifndef NDEBUG
...
void LOG_WARN(const char *app_file, const char *app_func, unsigned app_line, int rank, const char *str) {
#ifndef NDEBUG
...
void LOG_TRACE(const char *app_file, const char *app_func, unsigned app_line, int rank, const char *str) {
#ifndef NDEBUG
  struct timeval now;
  gettimeofday(&now, NULL);
